    ...
    <section className="stock-panel">
      ...
      <div className="table-wrap">
        <table className="stock-movement-table">
          <thead><tr><th>Data</th><th>Produto</th><th>Origem</th><th>Documento / motivo</th><th>De / Para</th><th>Entrada</th><th>Saída</th><th>Saldo após</th>{comAcoes ? <th>Ações</th> : null}</tr></thead>
          <tbody>
            {movimentos.length === 0 ? <tr><td colSpan={comAcoes ? 9 : 8} className="empty-state">Nenhuma movimentação no período.</td></tr> : movimentos.map(item => (
