...
interface Props { movimentos: MovimentoEstoqueCalculado[] }
const ORIGEM = { compra: 'Compra', venda: 'Venda', ajuste: 'Ajuste manual' }
function qtd(valor: number) { return valor.toLocaleString('pt-BR') }

export default function RelatorioMovimentosEstoque({ movimentos }: Props) {
  return (
    <section className="stock-panel">
      ...
      <div className="table-wrap">
        <table className="stock-movement-table">
          <thead><tr><th>Data</th><th>Produto</th><th>Origem</th><th>Documento / motivo</th><th>Entrada</th><th>Saída</th><th>Saldo após</th></tr></thead>
          <tbody>
            {movimentos.length === 0 ? <tr><td colSpan={7} className="empty-state">Nenhuma movimentação no período.</td></tr> : movimentos.map(item => (
