...
import type { Fornecedor, Unidade } from '@/types'
...
export default function FornecedoresPage() {
  ...
  async function load() {
    setLoading(true)
    let q = sb.from('btx_fornecedores').select('*').eq('ativo', true).order('nome')
  ...
  function openNew() {
    setForm(EMPTY)
    setFormUnidade(unidadeAtiva ?? (isAdmin ? '' : ''))
    setEditId(null); setErr(''); setModal(true)
  }
  function openEdit(r: Fornecedor) {
  ...
  async function save() {
    ...
    if (editId) await sb.from('btx_fornecedores').update(payload).eq('id', editId)
    else await sb.from('btx_fornecedores').insert(payload)
    setSaving(false); setModal(false); load()
  }

  async function remove(id: string) {
    setSaving(true)
    await sb.from('btx_fornecedores').update({ ativo: false }).eq('id', id)
  ...
  return (
    <div>
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            ...
            : rows.length === 0 ? <tr><td colSpan={5} className="empty-state">Nenhum fornecedor cadastrado.</td></tr>
            : rows.map(r => (
              <tr key={r.id}>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editId ? 'Editar fornecedor' : 'Novo fornecedor'}
        footer={<>
