...
export default function ParcelasPagarPage() {
  ...
  function formEditFromRow(r: Parcela) {
    ...
    return { vencimento: r.vencimento, valor: r.valor, forma_pagamento: r.forma_pagamento ?? ('boleto' as const), nf: nf && nf !== '—' ? nf : '', texto }
  }
  ...
  async function load() {
    ...
    for (const p of parcelas) {
      // conta do VHSYS (sem compra/despesa local vinculada) mostra a própria descrição
      const semVinculoLocal = !p.origem_id
      if (p.origem === 'compra' && !semVinculoLocal) {
        om.set(p.id, compraNome.get(p.origem_id!) || 'Compra')
        nf.set(p.id, compraNf.get(p.origem_id!) || '—')
      } else if (p.origem === 'despesa' && !semVinculoLocal) {
        om.set(p.id, despesaTxt.get(p.origem_id!) || 'Despesa')
        nf.set(p.id, despesaNf.get(p.origem_id!) || '—')
      } else {
        om.set(p.id, p.observacoes?.trim() || '—')
      ...
      {verRow && (
        ...
        >
          ...
          <div className="grid-2">
            ...
            <div className="form-group">
              <label className="form-label" style={{ color: 'var(--green)' }}>Pago em</label>
              <div className="form-input mono" style={{ background: 'var(--surface2)', color: verRow.status === 'pago' ? 'var(--green)' : 'var(--text-muted)' }}>
                {verRow.status === 'pago' ? formatData(verRow.data_pagamento) : '— ainda não pago'}
              </div>
            </div>
          </div>
          <div className="form-group">
